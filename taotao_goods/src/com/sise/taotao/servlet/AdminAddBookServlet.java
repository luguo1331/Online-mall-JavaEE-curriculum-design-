...
/*
 * 类名称: AdminAddBookServlet   
 * 类描述:                
 * 创建人: 凌威      
 * 修改人:  
 * 修改时间:2017-6-14 下午3:51:24 
 * 修改备注:
 * @version 1.0.0
 */
public class AdminAddBookServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 把图片的路径设置给book对象
		goods.setImage_1("images/" + filename);
		...
		// 把图片的路径设置给book对象
		goods.setImage_2("images/" + filename);
		
		
		// 获取文件名
		fileItem = fileItemList.get(3);//获取小图
		filename = fileItem.getName();
		// 截取文件名，因为部分浏览器上传的绝对路径
		index = filename.lastIndexOf("\\");
		if(index != -1) {
		...
		// 把图片的路径设置给book对象
		goods.setImage_3("images/" + filename);
		
		// 获取文件名
		fileItem = fileItemList.get(4);//获取小图
		filename = fileItem.getName();
		// 截取文件名，因为部分浏览器上传的绝对路径
		index = filename.lastIndexOf("\\");
		if(index != -1) {
		...
		// 把图片的路径设置给book对象
		goods.setImage_4("images/" + filename);
		
		
		// 调用service完成保存
		goods.setGid(CommonUtils.uuid());
		GoodsServlce goodsServlce = new GoodsServlce();
		goodsServlce.add(goods);
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/admin/addGoods.jsp").
